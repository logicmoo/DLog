...
parseXML(FileNameOrStream, EndState, StartState, Options, Module, Pred, F0,
	 ErrGoal) :-
	process_options(Options, DTD, ErrGoal, F0, SAXFlags),
	parseFileOrStream(FileNameOrStream, SAXFlags, EndState, StartState,
			  RDTD, Pred, Module),
	normal_dtd(RDTD, [], DTD).

parseFileOrStream(FileNameOrStream, SAXFlags, EndState, StartState, RDTD,
		   Pred, Module) :-
	current_stream(_FileName, _Mode, FileNameOrStream), !,
	stream_code(FileNameOrStream, StreamCode),
	c_parseXMLFromStream(StreamCode, SAXFlags, EndState, StartState, RDTD,
			     Pred, Module).
parseFileOrStream(FileName, SAXFlags, EndState, StartState, RDTD,
		  Pred, Module) :-
	absolute_file_name(FileName, AbsFileName, [access(read)]),
	c_parseXMLFromFile(AbsFileName, SAXFlags, EndState, StartState, RDTD,
			   Pred, Module).
...
foreign(parseXMLFromFile, c_parseXMLFromFile(+string,
					     +integer,[-term],+term,
					     -term,+atom,+atom)).
foreign(parseXMLFromStream, c_parseXMLFromStream(+address('SP_stream'),
						 +integer,[-term],+term,
						 -term,+atom,+atom)).

foreign_resource(sax, [parseXMLFromFile,parseXMLFromStream]).

%load_foreign_part_conditionally :-
%	user:file_search_path(env, _), !,
%	load_foreign_resource(env('lib/sax')).
%load_foreign_part_conditionally.
%
%:- load_foreign_part_conditionally.
