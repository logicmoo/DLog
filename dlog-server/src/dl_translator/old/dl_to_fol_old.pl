:- module(dl_to_fol_old, [axiomsToNNFConcepts/2, def_list/3, defNormForms/3, toFOLList/3, list_cls/2, negNormForm/2, clause_types/2]).
...
def2(C,Prefix,Pol,Q,Defs):-
	...
	defName(C,Prefix,Q),
	(
	  Pol = 1 -> Defs = [or([not(Q),C])]
	; Pol = -1 -> neg(C,NC), Defs = [or([Q,NC])]
	).
...
not_embedded(aconcept(_)):- !.
not_embedded(nconcept(_)):- !.
