:- module(saturate,[saturate/2, saturate_partially/3, resolve/3, redundant/2, remove_redundant/2, elim_reds/3, simplifyClauses/2, simplifyClause/2, find_unifiable/2, replace_occurrence_list/4, decomposeClause/2, remove_temp/2]).
:- use_module(show).
:- use_module(struct).
:- use_module(selectResolvable).
:- use_module(library(lists),[append/3,member/2,select/3,reverse/2,delete/3,sublist/2]).
